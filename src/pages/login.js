...
document.getElementById('quickAdminLogin').addEventListener('click', async () => {
  try {
    ...
    if (result.error) {
      showLoginError('Quick Admin Login failed', result.error);
      return;
    }
    window.location.href = 'index.html';
  } catch (error) {
    showLoginError('Quick Admin Login failed', error);
  }
});

document.getElementById('quickUser1Login').addEventListener('click', async () => {
  try {
    const result = await loginUser('user1@example.com', 'password123');
    if (result.error) {
      showLoginError('Quick User 1 Login failed', result.error);
      return;
    }
    window.location.href = 'index.html';
  } catch (error) {
    showLoginError('Quick User 1 Login failed', error);
  }
});

document.getElementById('quickUser2Login').addEventListener('click', async () => {
  try {
    const result = await loginUser('user2@example.com', 'password123');
    if (result.error) {
      showLoginError('Quick User 2 Login failed', result.error);
      return;
    }
    window.location.href = 'index.html';
  } catch (error) {
    showLoginError('Quick User 2 Login failed', error);
  }
});
